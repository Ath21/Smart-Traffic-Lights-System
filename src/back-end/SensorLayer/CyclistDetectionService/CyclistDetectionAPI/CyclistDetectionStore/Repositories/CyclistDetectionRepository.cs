...
{
    public class CyclistDetectionRepository : ICyclistDetectionRepository
    {
        private readonly DetectionDbContext _context;

        public CyclistDetectionRepository(DetectionDbContext context)
        {
            _context = context;
        }

        /// <summary>
        /// Inserts a new cyclist detection record into InfluxDB
        /// </summary>
        public async Task<Guid> InsertAsync(CyclistDetection detection)
        {
            detection.DetectionId = Guid.NewGuid();

            var point = PointData
                .Measurement("cyclist_detections")
        ...
        /// <summary>
        /// Queries cyclist detections from InfluxDB
        /// </summary>
        public async Task<List<CyclistDetection>> QueryAsync(
            ...
            int? limit = null)
        {
            ...
            var flux = $"from(bucket: \"{_context.Bucket}\")" +
                       ...
                       $" |> filter(fn: (r) => r._measurement == \"cyclist_detections\")";
            ...
            var results = new List<CyclistDetection>();
