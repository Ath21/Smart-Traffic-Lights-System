...
{
    public class PedestrianDetectionRepository : IPedestrianDetectionRepository
    {
        private readonly DetectionDbContext _context;

        public PedestrianDetectionRepository(DetectionDbContext context)
        {
            _context = context;
        }

        /// <summary>
        /// Inserts a new pedestrian detection record into InfluxDB
        /// </summary>
        public async Task<Guid> InsertAsync(PedestrianDetection detection)
        {
            detection.DetectionId = Guid.NewGuid();

            var point = PointData
                .Measurement("pedestrian_detections")
        ...
        /// <summary>
        /// Queries pedestrian detections from InfluxDB
        /// </summary>
        public async Task<List<PedestrianDetection>> QueryAsync(
            ...
            int? limit = null)
        {
            ...
            var flux = $"from(bucket: \"{_context.Bucket}\")" +
                       ...
                       $" |> filter(fn: (r) => r._measurement == \"pedestrian_detections\")";
            ...
            var results = new List<PedestrianDetection>();
            ...
            foreach (var table in tables)
            {
                foreach (var record in table.Records)
                {
                    if (record.GetValue() != null && record.GetField() != null)
                    {
                        ...
                        if (existing == null)
                        {
                            existing = new PedestrianDetection
                            {
                        ...
                        {
                            case "count":
                                existing.Count = Convert.ToInt32(record.GetValue());
                                break;
                            case "detection_id":
