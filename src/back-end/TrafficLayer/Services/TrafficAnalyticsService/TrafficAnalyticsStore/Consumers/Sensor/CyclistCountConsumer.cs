...
public class CyclistCountConsumer : IConsumer<CyclistCountMessage>
{
    ...
    public async Task Consume(ConsumeContext<CyclistCountMessage> context)
    {
        ...
        await _logPublisher.PublishAuditAsync(
            "[CONSUMER][CYCLIST_COUNT]",
            $"Received cyclist count {msg.Count} at {msg.Intersection}",
        ...
        await _summaryBusiness.UpdateCountsAsync(summary, "Cyclist", msg.Count, 0, 0);

        // 2️⃣ Add to in-memory aggregation
        _aggregation.AddCyclistData(msg);
